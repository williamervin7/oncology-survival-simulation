...
### Added
- `src/config.py`: added `COX_COVARIATES` to centralize the primary Cox PH baseline covariate specification.
- `src/models/models.py`: updated `preprocessing()` to use the centralized `COX_COVARIATES` specification rather than hard-coding the primary predictor list.
- `src/models/models.py`: primary Cox PH preprocessing now includes Age, Sex, Stage, Marital status at diagnosis, Race, Regional nodes examined, and Year of diagnosis.
- `src/models/models.py`: chemotherapy is intentionally excluded from the primary Cox PH model because it is a post-diagnosis treatment variable rather than baseline information.
- `src/models/models.py`: Stage III preprocessing now explicitly uses Stage IIIA as the reference category and creates `Stage_IIIB` and `Stage_IIIC` indicators.
- `src/models/models.py`: added one-hot encoding for Marital status at diagnosis and Race, with one reference category omitted for each categorical variable.
- `src/models/models.py`: added `run_multivariable_model()` to assemble the duration/event columns with the preprocessed baseline covariates and fit the multivariable Cox PH model.
- `documents/covariate_selection.md`: expanded the analytical rationale for baseline covariate selection, including the decision to exclude chemotherapy from the primary prognostic model and the consideration of Year of diagnosis as a calendar-era covariate.
- Successfully fit the first multivariable Cox PH model using the prespecified baseline covariate set.

### Fixed
- Corrected the multivariable Cox model assembly so transformed predictor columns generated by `preprocessing()` are concatenated with the original `Time` and `Event` columns before fitting, rather than attempting to retrieve transformed columns from the unprocessed dataframe.
- Replaced implicit Stage reference-category handling with explicit Stage IIIA reference coding.
- Updated the model preprocessing to remove chemotherapy from the primary Cox specification following the baseline-information/temporal-ordering decision.

### Findings
- The multivariable Cox PH model successfully fit after complete-case handling on the Stage III cohort.
- The fitted model contains 13 encoded predictor coefficients representing the prespecified baseline covariates:
  - Age
  - Sex
  ...
  - Stage IIIB
  - Stage IIIC
  - Five marital-status indicators
  - Two race indicators
- Stage III substage remained strongly associated with overall survival after multivariable adjustment:
  - Stage IIIB vs IIIA: HR ≈ 1.85
  - Stage IIIC vs IIIA: HR ≈ 3.99
- Age was positively associated with hazard (HR ≈ 1.048 per additional year).
- Regional nodes examined showed a modest inverse association with hazard (HR ≈ 0.986 per additional node).
- Year of diagnosis showed a modest inverse association with hazard (HR ≈ 0.985 per additional year), supporting the need to evaluate calendar-time effects and the proportional hazards assumption rather than interpreting the coefficient in isolation.
- Statistical significance from the multivariable model will not be used as an automatic variable-selection mechanism; the primary covariate set was prespecified using domain knowledge and literature-supported rationale.

### Known issues / follow-ups
- Marital-status and race reference categories are currently determined by `drop_first=True`; the exact omitted reference categories need to be explicitly verified and documented before the Phase 1 model is frozen.
- Final complete-case sample size and event count for the multivariable Cox model need to be recorded as part of the Phase 1 model audit.
- Continuous functional-form assumptions need to be evaluated for Age, Regional nodes examined, and Year of diagnosis.
- Proportional hazards assumptions have not yet been evaluated for the multivariable model.
- The current model output should not yet be treated as the final Phase 1 model until PH diagnostics and functional-form checks are completed.
- Grade/differentiation remains unavailable in the current extract and remains an open candidate for future re-extraction.
- Sidedness is intentionally deferred from the current modeling pass.

### Planned (next session)
1. Verify and explicitly document the reference categories for Marital status and Race.
2. Record the final multivariable Cox dataset size, number of events, and number censored after complete-case handling.
3. Inspect the final Cox design matrix and verify that the encoded predictors exactly match the prespecified analytical specification.
4. Run global and covariate-level proportional hazards diagnostics using Schoenfeld residuals / `cph.check_assumptions()`.
5. Review visual diagnostics for potential PH violations, with particular attention to Year of diagnosis and other continuous covariates.
6. Evaluate the functional form of continuous predictors, especially Age, Regional nodes examined, and Year of diagnosis.
7. Apply the precommitted PH remedy hierarchy if violations are identified and document the rationale for any modification.
